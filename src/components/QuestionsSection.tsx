...
export default function QuestionsSection() {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        const fileInput = formData.get(FormField.FILE) as File;

        // Generate Client ID
        const submissionId = Math.random().toString().slice(
            ID_GENERATION.RANDOM_SLICE_START,
            ID_GENERATION.RANDOM_SLICE_END
        );
        ...
        try {
            ...
            if (fileInput && fileInput.size > 0) {
                // File size limit
                if (fileInput.size > FILE_SIZE_LIMITS.MAX_UPLOAD_SIZE) {
                ...
                if (!ALLOWED_UPLOAD_TYPES.includes(fileInput.type as any)) {
                ...
                if (!uploadRes.ok) {
                    throw new Error(VALIDATION_MESSAGES.UPLOAD_FAILED);
                }
            ...
            formData.forEach((value, key) => {
                if (key !== FormField.FILE) { // Don't send file binary to Formspree if using URL
                    formspreeData.append(key, value);
                }
            });
            if (fileUrl) {
                formspreeData.append(FormField.FILE_URL, fileUrl);
            }
        ...
        <section id="questions" className="section questions-section bg-gray-50 dark:bg-gray-800 transition-colors duration-300 relative">
            ...
            <div className="container mx-auto px-6 py-12">
                ...
                >
                        ...
                        if (field.type === InputType.SELECT) {
                            ...
                            return (
                                <label key={field.name} className="block mb-4">
                                    ...
                                    <input
                                        type={InputType.FILE}
                                        name={FormField.FILE} // Ensure name is 'file' for backend
                                        accept={field.accept}
                    ...
                    >
                        {submitting ? BUTTON_TEXT.SUBMIT.submitting : questionsData.submitLabel}
                    </button>
                </form>
            </div>
        </section>
